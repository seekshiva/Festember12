<!doctype html>
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />  
    <title>Festember</title>
    <link rel="stylesheet" href="css/main.css" type="text/css" />    
  </head>
  <body>
  <noscript> 
    <style type="text/css"> body{display:none;} </style> 
	<div class="noscriptmsg" onclick="window.location.reload()"> Enable Javascript in your browser and refresh the page </div> 
  </noscript>  
    <div id="container_canvas"></div>
    <div id="overlay_container" class="overlay_container"></div>
    <div id="initial_container"></div>	
    <div id="brochure">
      <div id="brochure_sub">
	<img src="images/Festi.png" />
	<div style="width:150px; height:90px; margin-left:180px; padding-bottom:30px; margin-top:-30px; text-align:center; ">Powered By <img src="images/sponsors/q1.png" style="margin:0px; margin-left:-37px; width:150px; height:90px;"/></div>
	<div id="links">
	  <div style="float: right; width: 300px; ">
	    <div class="heading">Links:</div>
	    <a  target="_blank" href="https://www.facebook.com/festember">Facebook</a>
	    <a  target="_blank" href="https://www.twitter.com/festember">Twitter</a>
	    <a  target="_blank" href="http://en.wikipedia.org/wiki/Festember">Wikipedia</a>
	    <a  target="_blank" href="http://www.nitt.edu/home/students/events/festember/">About F-12</a>
	  </div>
	  <div style="float: left; width: 200px; ">
	    <div class="heading">Archives:</div>
	    <a style="width: 150px;" target="_blank"  href="https://festember.in/11/">Festember 11</a>
	    <a style="width: 150px;" target="_blank" href="https://festember.com/10/">Festember X</a>
	  </div>
	</div>
	<div style="text-align: center; margin-top: 200px; margin-left: 100px;">
	  <div class="fb-like-box" data-href="http://www.facebook.com/festember" data-width="292" data-show-faces="false" data-stream="false" data-header="true"></div>
	</div>
	<div id="fb-root"></div>
	<script>(function(d, s, id) {
	  var js, fjs = d.getElementsByTagName(s)[0];
	  if (d.getElementById(id)) return;
	  js = d.createElement(s); js.id = id;
	  js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=297426703670139";
	  fjs.parentNode.insertBefore(js, fjs);
	  }(document, 'script', 'facebook-jssdk'));</script>
      </div>
    </div>
    
	
    <div class="overlay" id="overlayed">
      <div id="contacts">	   
	<h2>Contact Us</h2>
	<div class="contact fl-l">
	  <div class="name">Abhishek Shah</div>
	  <div class="desig">Treasurer</div>
	  <div class="mobile">+91 98947 13840</div>
	  <div class="email">shah.abhisekh@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">D Dinesh Kumar</div>
	  <div class="desig">Overall Coordinator</div>
	  <div class="mobile">+91 95667 63918</div>
	  <div class="email">prody@festember.com</div>
	</div>
	<div class="contact">
	  <div class="name">Gaurav Kumar</div>
	  <div class="desig">Chairman</div>
	  <div class="mobile">+91 81442 25258</div>
	  <div class="email">gaurav@festember.com</div>
	</div>
	<div class="breaker" style="height: 40px; "></div>
	
	<div class="contact fl-l">
	  <div class="name">Vignesh B</div>
	  <div class="desig">Head, Marketing</div>
	  <div class="mobile">+91 99627 55477</div>
	  <div class="email">vignesh@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Sriram Manohar</div>
	  <div class="desig">Head, Marketing</div>
	  <div class="mobile">+91 96001 37932</div>
	  <div class="email">sriram@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <div class="name">Ashwin Kumar</div>
	  <div class="desig">Cultural Secretary</div>
	  <div class="mobile">+91 95002 65760</div>
	  <div class="email">psyches@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Pankaaj Padmanabhan</div>
	  <div class="desig">Head, Events</div>
	  <div class="mobile">+91 90425 26559</div>
	  <div class="email">pankaaj@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <div class="name">Utkarsh Jaiswal</div>
	  <div class="desig">Head, Media</div>
	  <div class="mobile">+91 99529 80706</div>
	  <div class="email">utkarsh@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Sashank MV</div>
	  <div class="desig">Head, Publicity</div>
	  <div class="mobile">+91 97878 33728</div>
	  <div class="email">sashank@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <div class="name">Abhishek S</div>
	  <div class="desig">Head, Public Relations</div>
	  <div class="mobile">+91 91593 11992</div>
	  <div class="email">abs@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Jeh Agarwal</div>
	  <div class="desig">Head, Workshops</div>
	  <div class="mobile">+91 95669 39866</div>
	  <div class="email">jeh@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <div class="name">Vivek Chandru</div>
	  <div class="desig">Head, Organising</div>
	  <div class="mobile">+91 95664 60216</div>
	  <div class="email">vivek@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Koushik Sri</div>
	  <div class="desig">Head, Hospitality</div>
	  <div class="mobile">+91 96772 16811</div>
	  <div class="email">ks@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <div class="name">V Shiva Nandan</div>
	  <div class="desig">Head, CSG</div>
	  <div class="mobile">+91 95006 81056</div>
	  <div class="email">seek@festember.com</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Hari Kumaran Babu</div>
	  <div class="desig">Head, Design</div>
	  <div class="mobile">+91 81488 18451</div>
	  <div class="email">hkbsaga@festember.com</div>
	</div>
	<div class="breaker"></div>
	
	<div class="contact fl-l">
	  <h3 class="desig">Our Patron</h3>
	  <div class="name">Dr. Srinivasan Sundarrajan</div>
	  <div class="title">Director, NIT Tiruchirappalli</div>
	</div>
	<div class="contact fl-r">
	  <h3 class="desig">Convener</h3>
	  <div class="name">Dr. K. Sankaranarayanasamy</div>
	  <div class="title">Dean Students</div>
	</div>
	<div class="breaker" style="height: 150px; "></div>
	
	<h2>Faculty Advisors</h2>
	<div class="contact fl-l">
	  <div class="name">Dr. P. Parthiban</div>
	  <div class="title">Asst. Professor</div>
	  <div class="dept">Dept. of Production Engineering</div>
	</div>
	<div class="contact fl-r">
	  <div class="name">Dr. T. Ramesh</div>
	  <div class="title">Asst. Professor</div>
	  <div class="dept">Dept. of Mechanical Engineering</div>
	</div>
	<div class="breaker"></div>
      </div>

      <div id="sponsors">
	<div id="partner">
	  <div align="center" style="margin-bottom: 30px; ">
	    <img style="border: none; box-shadow: none;" src="./images/wannabpartner.png" width="200px"><br />
	    <div>Contact us at: <span class="email">marketing@festember.com</span></div>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Powered By</div>
	    <a href="http://www.nimbuzz.com" target="_blank"><img src="/12/images/sponsors/q1.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Telecom Partner</div>
	    <a href="http://www.aircel.com" target="_blank"><img src="/12/images/sponsors/q2.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Beverage Partner</div>
	    <a href="http://www.coca-cola.com" target="_blank"><img src="/12/images/sponsors/q3.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Curtain Raise Title Partner</div>
	    <a href="http://in.asus.com" target="_blank"><img src="/12/images/sponsors/q4.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Educational Partner</div>
	    <a href="http://byjusclasses.com/" target="_blank"><img src="/12/images/sponsors/q5.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Refreshment Partner</div>
	    <a href="http://www.appyfizz.com" target="_blank"><img src="/12/images/sponsors/q6.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Snacking Partner</div>
	    <a href="http://parleagro.com/Hippo.html" target="_blank"><img src="/12/images/sponsors/q7.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Alumni Partner</div>
	    <a href="http://www.innovativeadhesive.com/" target="_blank"><img src="/12/images/sponsors/q8.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Official Music Partner</div>
	    <a href="http://www.sam.org.in/" target="_blank"><img src="/12/images/sponsors/q9.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Prize Partner</div>
	    <a href="http://www.tritonmusicz.com/" target="_blank"><img src="/12/images/sponsors/q10.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Logistics Partner</div>
	    <a href="http://www.dhl.co.in" target="_blank"><img src="/12/images/sponsors/q11.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Workshops Partner</div>
	    <a href="http://www.adormi.com" target="_blank"><img src="/12/images/sponsors/q12.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Dramatics Partner</div>
	    <a href="http://www.mytindia.com" target="_blank"><img src="/12/images/sponsors/q13.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Digital Media Partner</div>
	    <a href="http://www.dsnglobal.com" target="_blank"><img src="/12/images/sponsors/q14.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Fashion Partner</div>
	    <a href="http://www.flyingmachine.co.in" target="_blank"><img src="/12/images/sponsors/q15.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Amusement Park Partner</div>
	    <a href="http://www.btpark.org/home.html" target="_blank"><img src="/12/images/sponsors/q16.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Online Metal Promotion Partner</div>
	    <a href="http://www.indianstage.in" target="_blank"><img src="/12/images/sponsors/q17.png"></a>
	    <a href="http://www.metalbase.in" target="_blank"><img src="/12/images/sponsors/q18.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Online MBA Partner</div>
	    <a href="http://www.testfunda.com" target="_blank"><img src="/12/images/sponsors/q19.png"></a>
	  </div>
	  <div class="sponsor_c">
	    <div class="sponsor_type">Online Media Partner</div>
	    <a href="http://www.collegekhabar.com" target="_blank"><img src="/12/images/sponsors/q20.png"></a>
	    <a href="http://www.freshersworld.com" target="_blank"><img src="/12/images/sponsors/q21.png"></a>
	    <a href="http://www.knowafest.com" target="_blank"><img src="/12/images/sponsors/q22.png"></a>
	    <a href="http://www.twenty19.com" target="_blank"><img src="/12/images/sponsors/q23.png"></a>
	    <a href="http://www.fadooengineers.com" target="_blank"><img src="/12/images/sponsors/q24.png"></a>
	    <a href="http://www.jammag.com" target="_blank"><img src="/12/images/sponsors/q25.png"></a>
	    <a href="http://www.markmyfest.com" target="_blank"><img src="/12/images/sponsors/q26.png"></a>
	    <a href="http://youthconnectmagcom" target="_blank"><img src="/12/images/sponsors/q27.png"></a>
	  </div>
	</div>
      </div>
      
      <div id="events" style="text-align: left; ">
	<h2 class="event-head">Events</h2>
		<div id="header"> 
		<ul>
			<li class="selected"><a href="#">Lits</a></li>
			<li><a href="#">Choreo</a></li>
			<li><a href="#">Music</a></li>
			<li><a href="#">Arts</a></li>
		</ul>

		</div>

		<div id="content">

      </div>
      </div>
      
      <div id="workshops" style="text-align: left; ">
	<h2 style="text-align: center; ">Workshops</h2>
	<p>Workshops are much sought-after in Festember. It is not just the thrilling neck to neck events, pro-shows and DJ nights that the students like the most. Festember has made a great impression with its workshops which give it an edge over the other cultural events organised elsewhere in the country. Workshops at Festember always guarantees its participants a fun filled experience along with learning. Though they last for a few hours, they still make a memorable mark in the minds of the participants.</p>
	<p>Some of the workshops we have had in the past are:</p>
	<ul>
	  <li>Fencing</li>
	  <li>Archery</li>
	  <li>Hypnotism</li>
	  <li>Flair Bartending</li>
	  <li>Ventriloquism</li>
	  <li>Mente Magica</li>
	</ul>
	<p>This year, our team has taken up the responsibility to continue this tradition and ensure that the workshops live up to the expectations as always. So watch out for some exciting fun filled workshops this time!</p>
    
	</div>
     
      <div id="informals" style="text-align: left; ">
	<h2 style="text-align: center; ">Informals</h2>
	<p>Paintballing (where you and a bunch of your friends can team up, strategize and play against another team), Shooting andArchery (target events), Zorbing (Two people are put into a giant rubber inflated ball,and rolled around), Talent hunts, etc... need any more be said!! And at a much lower rate than they normally come! The Informals are quite literally at the heart of what makes Festember such a success, besides the loads of competitions. They form a platform for you to meet and make friends with people you don't know, and take back glorious memories. The RETRO informals promise to be bigger than life, and take you places you would never have dreamt of going.</p>
      </div>	
      
      <div id="nothing">
	<p><b>Coming Up Soon!</b></p>
      </div>	
    </div>
    
    <div id="container">
      <div id="pronite_container">
	<canvas id="element_pronite" class="element_canvas"></canvas>
      </div>
      
      <div id="casette_container">
	<canvas id="element_casette" class="element_canvas"></canvas>	
	<div id="theme_play" class="play-pause">
	  <img src="images/play.png">
	</div>
	<div id="theme_pause" class="play-pause">
	  <img src="images/pause.png">
	</div>
      </div>
      
      <img src="images/casste.png" class="player" id="player"/>
      <div id="contact_container">
	<canvas id="element_contact" class="element_canvas"></canvas>	
      </div>
      <div id="sponsor_container">
	<img src="images/sponsors_skewed/q1.png">
	<img src="images/sponsors_skewed/q2.png">
	<img src="images/sponsors_skewed/q3.png">
	<img src="images/sponsors_skewed/q4.png">
	<img src="images/sponsors_skewed/q5.png">
	<img src="images/sponsors_skewed/q6.png">
	<img src="images/sponsors_skewed/q7.png">
	<img src="images/sponsors_skewed/q8.png">
	<img src="images/sponsors_skewed/q9.png">
	<img src="images/sponsors_skewed/q10.png">
	<img src="images/sponsors_skewed/q11.png">
	<img src="images/sponsors_skewed/q12.png">
	<img src="images/sponsors_skewed/q13.png">
	<img src="images/sponsors_skewed/q14.png">
	<img src="images/sponsors_skewed/q15.png">
	<img src="images/sponsors_skewed/q16.png">
	<img src="images/sponsors_skewed/q17.png">
	<img src="images/sponsors_skewed/q18.png">
	<img src="images/sponsors_skewed/q19.png">
	<img src="images/sponsors_skewed/q20.png">
	<img src="images/sponsors_skewed/q21.png">
	<img src="images/sponsors_skewed/q22.png">
	<img src="images/sponsors_skewed/q23.png">
	<img src="images/sponsors_skewed/q24.png">
	<img src="images/sponsors_skewed/q25.png">
	<img src="images/sponsors_skewed/q26.png">
	<img src="images/sponsors_skewed/q27.png">
      </div>
	  <div id="sponsor_cover_container">
	  </div>
      <div id="games_container">
	<canvas id="element_games" class="element_canvas"></canvas>		
      </div>	
      <div id="workshop_container"></div>	
      
      <div id="element_workshop" class="element_canvas"></div>	
      <div id="element_workshop_1" class="element_canvas"></div>		
      <div id="element_workshop_2" class="element_canvas"></div>	
      <div id="element_workshop_3" class="element_canvas"></div>			  
      <div id="element_workshop_4" class="element_canvas"></div>		  
      <div id="element_events_1" class="element_canvas"></div>			   
      <div id="element_events_2" class="element_canvas"></div>			   
      <div id="element_events_3" class="element_canvas"></div>			   
      <div id="element_events_4" class="element_canvas"></div>			   
      <div id="element_events_5" class="element_canvas"></div>			   
      <div id="element_events_6" class="element_canvas"></div>			   
      <div id="element_events_7" class="element_canvas"></div>			   
      
      <div id="events_container"></div> 
      <div id="house_1"></div>
      <div id="house_2"></div>
      <div id="house_3"></div>
      <div id="house_4"></div>	
      <div id="house_5"></div>
      <div id="house_6"></div>	
      <div id="house_7"></div>	
      <div id="house_8"></div>		
      <div id="house_9"></div>	
      <div id="house_10" class="countdown">
	<p id="time"><b></b></p><br><br><br>
      </div>	
      <div id="house_11" class="streetlight"></div>
      <div id="cloud-holder">	  
	<div id="sun"></div>
	<div id="moon"></div>
      </div>
      <div id="informals_container">
	<img src="./images/giantwheel1.png">
      </div>
    </div>
   <img src="images/street.png" id="container_image"/>  	
    <script>
      var _gaq = _gaq || [];
      _gaq.push(['_setAccount', 'UA-32752974-1']);
      _gaq.push(['_setDomainName', 'festember.com']);
      _gaq.push(['_trackPageview']);
      
      function updateAnalytics() {
      (function() {
      var ga = document.createElement('script'); ga.type = 'text/javascript'; ga.async = true;
      ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';
      var s = document.getElementsByTagName('script')[0]; s.parentNode.insertBefore(ga, s);
      })();
      }
      updateAnalytics();
    </script>
    <script src="js/jquery.js" type="text/javascript"></script>
    <script src="js/debounce.js" type="text/javascript"></script>
    <script src="js/event_content.js" type="text/javascript"></script>	
    <script type="text/javascript" src="js/main.js"></script>
  </body>
</html>
